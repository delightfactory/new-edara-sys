...
- contribution parity must remain valid;
...
The canonical guard `ai_ops.current_decision_issues(decision_id)` now dispatches by the frozen case domain.
...
The private bridge now dispatches by frozen domain.
...
The TypeScript worker response now supports `snapshot.domain_captures` while retaining the legacy singular Receivables capture field.
...
Branch-level contract tests cover:
...
These tests do **not** replace isolated database runtime acceptance.

Still required before this Sales vertical slice is called runtime-complete:

1. apply the full migration chain to an isolated/local Supabase/PostgreSQL environment;
2. prove the existing Sales freshness scenarios with real-like target data;
3. prove a two-domain snapshot respects the global case budget and immutable ranks;
4. prove worker context parsing/hashing and zero-case combinations at runtime;
5. stage and validate mixed Credit + Sales decisions in one run;
6. prove stale Sales progress blocks action but does not hide the case;
7. approve and commit exactly one Sales decision to exactly one system-origin Work item linked to its target;
8. retry that commit and prove no duplicate Work;
9. mutate target/progress/context/Work between review and commit and prove commit-time rejection;
10. rerun the complete repository CI and independent technical/security review.

Until those gates pass, this remains a reviewed branch implementation, not a production release.

## 10. Expansion rule

Do not begin Customer Reactivation merely because the Sales SQL exists. The Sales vertical slice should first pass the applicable isolated runtime acceptance above. Once it does, Customer Health / Re-engagement can reuse the same multi-domain worker, validation dispatcher and reviewed Work bridge pattern instead of creating another parallel AI subsystem.
