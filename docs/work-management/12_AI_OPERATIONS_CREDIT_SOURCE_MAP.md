...
**Source inspected:** live production catalog, read-only  
...
`assigned_rep_id` is the strongest current operational sales-owner signal for a customer. It is not by itself proof of who caused a credit condition.
...
This is supporting context for credit-policy changes. It is not required for the first overdue-invoice candidate query and therefore should be drilled into only when needed.
...
The deployed `public.get_overdue_sales_invoices(...)` already uses these factual rules:

- order status in `delivered`, `partially_delivered`;
- payment terms in `credit`, `mixed`;
- delivery date exists;
- due date exists and is before the current date;
...
The AI layer should preserve the same business meaning, while replacing implicit UTC `CURRENT_DATE` with the run's explicit Cairo `business_date` for deterministic replay.
...
- who changed the collection deadline and why.
