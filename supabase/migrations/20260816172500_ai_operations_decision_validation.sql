-- ============================================================================
-- AI Operations Planner — Staged Decision Validation Gate
--
...
--
-- Purpose:
--   A staged AI recommendation is NOT execution authority. Before any later
--   human approval or Work bridge can act, compare decision-time frozen evidence
--   with current operational reality and fail closed on meaningful drift.
--
-- This migration mutates only ai_ops decision/run validation state.
-- It never mutates Sales, Customers, Credit, Work, HR or Inventory.
-- ============================================================================
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'receivables capture marker not found';
  END IF;
  ...
  LOOP
    ...
    IF NOT FOUND THEN
      v_reasons := v_reasons || jsonb_build_array('frozen_case_evidence_missing');
    ELSIF v_sc.domain <> 'receivables' OR v_sc.case_type <> 'overdue_invoice' THEN
      v_reasons := v_reasons || jsonb_build_array('unsupported_validation_domain');
    ELSE
      SELECT
        so.id,
        so.customer_id,
      ...
      IF NOT FOUND THEN
        v_reasons := v_reasons || jsonb_build_array('sales_order_missing');
      ELSE
        ...
        IF EXISTS (
          ...
          WHERE h.order_id = v_current_order.id
            AND h.created_at > v_sc.created_at
        ) THEN
        ...
        IF EXISTS (
          ...
          WHERE h.customer_id = v_current_order.customer_id
            AND h.created_at > v_sc.created_at
        ) THEN
    ...
    -- A partial/blocked capture may still support passive human-facing decisions,
    -- but it is insufficient to authorize an action recommendation downstream.
...
COMMENT ON FUNCTION ai_ops.validate_staged_run(UUID) IS
  'Internal fail-closed validation of staged decisions against current receivables, credit governance, operational context, active Work collisions and actor availability. Mutates ai_ops validation state only; performs no operational action.';
