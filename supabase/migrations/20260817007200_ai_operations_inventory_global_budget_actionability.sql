-- ============================================================================
-- AI Operations Planner — Four-Domain Global Budget + Inventory Actionability
--
-- DESIGN-TIME MIGRATION ONLY.
-- Adds Inventory to the one hard Case budget without comparing severity scales
-- across domains, and extends the shared bounded-partial actionability contract
-- only for the explicitly known inventory-v1 capture format.
-- ============================================================================
SET lock_timeout = '5s';
SET statement_timeout = '60s';

ALTER FUNCTION ai_ops.build_operational_snapshot(UUID,INTEGER)
  RENAME TO build_operational_snapshot_three_domain_v1;
REVOKE ALL ON FUNCTION ai_ops.build_operational_snapshot_three_domain_v1(UUID,INTEGER)
...
DECLARE
  ...
  v_inventory_has_candidate BOOLEAN:=false;
  v_three_domain_build JSONB;
  v_inventory_build JSONB;
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF v_credit_capture.snapshot_id IS NULL
      ...
      RAISE EXCEPTION 'existing operational snapshot is missing a required pre-Inventory domain capture marker';
    END IF;

    IF v_inventory_capture.snapshot_id IS NOT NULL THEN
      ...
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'evidence_bytes',dc.evidence_bytes,'capture_version',dc.capture_version,'metadata',dc.metadata
                               ...
                               WHEN 'customer_health' THEN 2 WHEN 'inventory' THEN 3 ELSE 4 END,dc.domain),'[]'::JSONB)
    ...
    IF v_context_bound THEN
      RAISE EXCEPTION 'existing snapshot is already bound to a worker context and cannot add Inventory';
    END IF;

    v_existing_cases:=v_credit_capture.case_count+v_sales_capture.case_count+v_customer_capture.case_count;
    ...
    v_inventory_alloc:=LEAST(v_inventory_demand,v_remaining);
  ELSE
    ...
    v_allocations:=ai_ops.allocate_domain_case_budget(
      ...
      ARRAY['receivables','sales','customer_health','inventory']::TEXT[]
    );
    ...
    -- Preserve the reviewed three-domain builder inside its reserved share.
    -- Any unused share may spill to Inventory only by remaining capacity, never
    -- by cross-domain severity ordering.
    v_three_domain_build:=ai_ops.build_operational_snapshot_three_domain_v1(
      p_run_id,GREATEST(v_existing_domain_budget,1)
    );

    SELECT * INTO v_snapshot FROM ai_ops.snapshots WHERE run_id=p_run_id;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'snapshot missing after preserved three-domain base build';
    END IF;
    ...
    IF v_credit_capture.snapshot_id IS NULL OR v_sales_capture.snapshot_id IS NULL
       OR v_customer_capture.snapshot_id IS NULL THEN
      RAISE EXCEPTION 'preserved three-domain builder did not produce required capture markers';
    END IF;
  ...
  IF v_inventory_alloc>0 THEN
    v_inventory_build:=ai_ops.refresh_inventory_cases(
      v_snapshot.id,v_run.business_date,v_inventory_alloc
    );
  ELSE
    v_inventory_has_candidate:=v_inventory_demand>0;
    INSERT INTO ai_ops.snapshot_domain_captures(
      ...
      v_snapshot.id,'inventory','inventory-v1',v_snapshot.data_as_of,v_run.business_date,
      0,0,CASE WHEN v_inventory_has_candidate THEN 'partial' ELSE 'completed' END,
      jsonb_build_object(
        'capture_marker_written',true,
        'case_types',jsonb_build_array('local_shortage','stalled_transfer'),
        'global_budget_exhausted',v_inventory_has_candidate,
        'global_case_limit',v_limit,'allocated_case_limit',0,'case_limit',0,
        'has_more',v_inventory_has_candidate,
  ...
  IF v_credit_capture.snapshot_id IS NULL OR v_sales_capture.snapshot_id IS NULL
    ...
    RAISE EXCEPTION 'operational snapshot did not produce all four immutable domain capture markers';
  END IF;
...
-- Preserve the already-reviewed three-domain bounded-partial gate and extend
-- only the explicitly known Inventory capture contract.
ALTER FUNCTION ai_ops.selected_case_capture_allows_action(UUID)
  RENAME TO selected_case_capture_allows_action_three_domain_v1;
REVOKE ALL ON FUNCTION ai_ops.selected_case_capture_allows_action_three_domain_v1(UUID)
  FROM PUBLIC,anon,authenticated,service_role;
...
BEGIN
  ...
  IF v_domain<>'inventory' THEN
    RETURN ai_ops.selected_case_capture_allows_action_three_domain_v1(p_decision_id);
  END IF;
  ...
  IF v_domain<>'inventory' OR v_capture_version<>'inventory-v1' THEN RETURN false; END IF;
  ...
  WHERE sc.snapshot_id=v_snapshot_id AND sc.domain='inventory';
...
COMMENT ON FUNCTION ai_ops.build_operational_snapshot(UUID,INTEGER) IS
  'Canonical four-domain operational snapshot builder for Receivables, Sales, Customer Health and Inventory under one hard domain-neutral Case budget.';
COMMENT ON FUNCTION ai_ops.selected_case_capture_allows_action(UUID) IS
  'Shared fail-closed selected-Case actionability gate extended to trusted inventory-v1 bounded partial captures while preserving the prior three-domain primitive.';
