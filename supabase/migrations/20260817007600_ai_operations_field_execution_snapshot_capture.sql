-- ============================================================================
-- AI Operations Planner — Field Execution Immutable Snapshot Capture
--
-- DESIGN-TIME MIGRATION ONLY. Field operational tables remain read-only.
-- Freezes overdue visit-day cases, explicit responsibility evidence and bounded
-- governed context into the shared immutable planner snapshot.
-- ============================================================================
SET lock_timeout = '5s';
SET statement_timeout = '60s';

CREATE OR REPLACE FUNCTION ai_ops.refresh_field_execution_cases(
  p_snapshot_id UUID,
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE dc.snapshot_id=p_snapshot_id AND dc.domain='field_execution'
  ) THEN
    RAISE EXCEPTION 'ai_ops field_execution domain already captured: %',p_snapshot_id;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'ai_ops field_execution evidence exists without capture marker: %',p_snapshot_id;
  END IF;
  ...
  INTO v_has_more
  FROM ai_ops.field_execution_candidates(p_business_date,LEAST(v_effective_limit+1,2000));
  ...
  FOR v_candidate IN
    SELECT * FROM ai_ops.field_execution_candidates(p_business_date,v_effective_limit)
  LOOP
    v_count:=v_count+1;
    ...
    INSERT INTO ai_ops.cases(
      ...
      v_candidate.case_key,p_snapshot_id,p_snapshot_id,'field_execution',
    ...
    WHERE oc.status='active'
      ...
      AND (
        (oc.subject_type='employee' AND oc.subject_id=v_candidate.rep_employee_id)
        OR (
    ...
    v_case_payload_bytes:=octet_length(convert_to(
      jsonb_build_object(
        'case_key',v_candidate.case_key,'domain','field_execution',
    ...
    INSERT INTO ai_ops.snapshot_cases(
      ...
      'field_execution',v_candidate.case_type,v_candidate.entity_type,v_candidate.entity_id,
  ...
  INSERT INTO ai_ops.snapshot_domain_captures(
    ...
    p_snapshot_id,'field_execution','field-execution-v1',
    ...
    jsonb_build_object(
      ...
      'overdue_visit_day_count',v_count,
      ...
      'case_granularity','visit_plan_day',
      'operational_plan_states',jsonb_build_array('confirmed','in_progress'),
      'exact_gps_captured',false,
      'visit_mutation_performed',false,'resolution_performed',false
    )
  );

  RETURN jsonb_build_object(
    'snapshot_id',p_snapshot_id,'business_date',p_business_date,
    'domain','field_execution','capture_status',v_capture_status,
    'candidates_upserted',v_count,'overdue_visit_day_count',v_count,
...
REVOKE ALL ON FUNCTION ai_ops.refresh_field_execution_cases(UUID,DATE,INTEGER)
  FROM PUBLIC,anon,authenticated,service_role;

COMMENT ON FUNCTION ai_ops.refresh_field_execution_cases(UUID,DATE,INTEGER) IS
  'Single-shot Field Execution capture freezing overdue visit-day commitments and bounded employee/branch context; field records remain read-only.';
