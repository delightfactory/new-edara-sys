-- ============================================================================
-- AI Operations Planner — Inventory Immutable Snapshot Capture
--
-- DESIGN-TIME MIGRATION ONLY. Inventory operational tables remain read-only.
-- Freezes deterministic Inventory cases plus bounded governed context into the
-- shared immutable planner snapshot.
-- ============================================================================
SET lock_timeout = '5s';
SET statement_timeout = '60s';

CREATE OR REPLACE FUNCTION ai_ops.refresh_inventory_cases(
  p_snapshot_id UUID,
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE dc.snapshot_id=p_snapshot_id AND dc.domain='inventory'
  ) THEN
    RAISE EXCEPTION 'ai_ops inventory domain already captured: %',p_snapshot_id;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'ai_ops inventory evidence exists without capture marker: %',p_snapshot_id;
  END IF;
  ...
  INTO v_has_more
  FROM ai_ops.inventory_candidates(p_business_date,LEAST(v_effective_limit+1,2000));
  ...
  FOR v_candidate IN
    SELECT * FROM ai_ops.inventory_candidates(p_business_date,v_effective_limit)
  LOOP
    v_count:=v_count+1;
    IF v_candidate.case_type='local_shortage' THEN
    ...
    INSERT INTO ai_ops.cases(
      ...
      v_candidate.case_key,p_snapshot_id,p_snapshot_id,'inventory',
    ...
    WHERE oc.status='active'
      ...
      AND (
        (oc.subject_type='warehouse' AND oc.subject_id=v_candidate.warehouse_id)
        OR (v_candidate.product_id IS NOT NULL AND oc.subject_type='product' AND oc.subject_id=v_candidate.product_id)
        OR (
    ...
    v_case_payload_bytes:=octet_length(convert_to(
      jsonb_build_object(
        'case_key',v_candidate.case_key,'domain','inventory',
    ...
    INSERT INTO ai_ops.snapshot_cases(
      ...
      'inventory',v_candidate.case_type,v_candidate.entity_type,v_candidate.entity_id,
  ...
  INSERT INTO ai_ops.snapshot_domain_captures(
    ...
    p_snapshot_id,'inventory','inventory-v1',v_snapshot.data_as_of,p_business_date,
    v_count,v_evidence_bytes,v_capture_status,
    jsonb_build_object(
      'case_types',jsonb_build_array('local_shortage','stalled_transfer'),
      'local_shortage_count',v_shortage_count,
      'stalled_transfer_count',v_transfer_count,
      ...
      'min_stock_level_authoritative',false,
      'batch_expiry_case_enabled',false,
  ...
  RETURN jsonb_build_object(
    'snapshot_id',p_snapshot_id,'business_date',p_business_date,'domain','inventory',
    'capture_status',v_capture_status,'candidates_upserted',v_count,
    'local_shortage_count',v_shortage_count,'stalled_transfer_count',v_transfer_count,
    'snapshot_evidence_rows',v_count,'snapshot_evidence_bytes',v_evidence_bytes,
    'context_rows_captured',v_context_rows,'context_truncated_cases',v_context_truncated_cases,
    'has_more',v_has_more,'capture_marker_written',true,
    'operational_mutation_performed',false,'resolution_performed',false
  );
END;
$$;

REVOKE ALL ON FUNCTION ai_ops.refresh_inventory_cases(UUID,DATE,INTEGER)
  FROM PUBLIC,anon,authenticated,service_role;

COMMENT ON FUNCTION ai_ops.refresh_inventory_cases(UUID,DATE,INTEGER) IS
  'Single-shot Inventory capture freezing demand-backed shortages and stalled transfers into the shared immutable snapshot.';
