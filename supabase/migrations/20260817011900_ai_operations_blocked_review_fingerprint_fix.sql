-- ============================================================================
-- AI Operations — preserve approval-blocked semantics under full fingerprinting
--
-- The canonical human-review path can return `approval_blocked=true` when fresh
-- current-state revalidation fails. That is a valid fail-closed outcome and does
-- not create a decision_reviews row. Full fingerprint binding applies only to a
-- successfully persisted human review; it must never turn a blocked approval
-- into an exception that rolls back the useful validation result.
-- ============================================================================
SET lock_timeout = '5s';
SET statement_timeout = '60s';

ALTER FUNCTION public.ai_ops_review_decision(UUID,TEXT,TEXT)
  RENAME TO ai_ops_review_decision_pre_blocked_semantics_fix_v1;
REVOKE ALL ON FUNCTION public.ai_ops_review_decision_pre_blocked_semantics_fix_v1(UUID,TEXT,TEXT)
...
BEGIN
  ...
  -- Call the canonical pre-full-fingerprint review path directly. It already
  -- owns current-state revalidation, human-review immutability and run lifecycle.
  v_result:=public.ai_ops_review_decision_pre_full_fingerprint_v1(
    p_decision_id,p_review_state,p_review_note
  );

  -- A current-state block is a successful safety outcome, not a persisted human
  -- review. Preserve it exactly and do not require a review binding.
...
COMMENT ON FUNCTION public.ai_ops_review_decision(UUID,TEXT,TEXT) IS
  'Final human-review gateway. Fresh current-state blocks remain non-exceptional fail-closed results; successfully persisted reviews receive an immutable full decision fingerprint.';
