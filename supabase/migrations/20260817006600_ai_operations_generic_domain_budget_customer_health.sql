-- ============================================================================
-- AI Operations Planner — Generic Domain Budget + Customer Health Integration
--
-- DESIGN-TIME MIGRATION ONLY.
--
-- Replaces the two-domain-only allocation assumption with a deterministic
-- round-robin water-fill helper that can accept additional domains without
-- comparing domain-specific severity scales. The existing proven Receivables
-- + Sales builder is preserved as a private primitive and Customer Health is
-- added before any worker context hash can bind to the snapshot.
-- ============================================================================

SET lock_timeout = '5s';
SET statement_timeout = '60s';

CREATE OR REPLACE FUNCTION ai_ops.allocate_domain_case_budget(
  p_limit INTEGER,
  p_demands JSONB,
  p_domain_order TEXT[]
)
RETURNS JSONB
LANGUAGE plpgsql
IMMUTABLE
SET search_path = ''
AS $$
DECLARE
  v_limit INTEGER := GREATEST(COALESCE(p_limit,0),0);
  v_remaining INTEGER;
  v_domain TEXT;
  v_demand INTEGER;
  v_allocated INTEGER;
  v_result JSONB := '{}'::JSONB;
  v_progress BOOLEAN;
BEGIN
  IF jsonb_typeof(COALESCE(p_demands,'{}'::JSONB))<>'object' THEN
    RAISE EXCEPTION 'domain demands must be a JSON object';
  END IF;
  IF p_domain_order IS NULL OR cardinality(p_domain_order)=0 THEN
    RETURN '{}'::JSONB;
  END IF;
  IF cardinality(p_domain_order)<>(
    SELECT COUNT(DISTINCT x) FROM unnest(p_domain_order) x
  ) THEN
    RAISE EXCEPTION 'domain order contains duplicates';
  END IF;

  FOREACH v_domain IN ARRAY p_domain_order LOOP
    IF v_domain IS NULL OR btrim(v_domain)='' THEN
      RAISE EXCEPTION 'domain order contains blank domain';
    END IF;
    v_result:=v_result||jsonb_build_object(v_domain,0);
  END LOOP;

  v_remaining:=v_limit;
  WHILE v_remaining>0 LOOP
    v_progress:=false;
    FOREACH v_domain IN ARRAY p_domain_order LOOP
      EXIT WHEN v_remaining<=0;
      v_demand:=GREATEST(COALESCE(NULLIF(p_demands->>v_domain,'')::INTEGER,0),0);
      v_allocated:=COALESCE((v_result->>v_domain)::INTEGER,0);
      IF v_allocated<v_demand THEN
        v_result:=jsonb_set(v_result,ARRAY[v_domain],to_jsonb(v_allocated+1),true);
        v_remaining:=v_remaining-1;
        v_progress:=true;
      END IF;
    END LOOP;
    EXIT WHEN NOT v_progress;
  END LOOP;

  RETURN v_result;
END;
$$;

REVOKE ALL ON FUNCTION ai_ops.allocate_domain_case_budget(INTEGER,JSONB,TEXT[])
  FROM PUBLIC,anon,authenticated,service_role;

ALTER FUNCTION ai_ops.build_operational_snapshot(UUID,INTEGER)
  RENAME TO build_operational_snapshot_two_domain_v1;

REVOKE ALL ON FUNCTION ai_ops.build_operational_snapshot_two_domain_v1(UUID,INTEGER)
  FROM PUBLIC,anon,authenticated,service_role;

CREATE OR REPLACE FUNCTION ai_ops.build_operational_snapshot(
  p_run_id UUID,
  p_case_limit INTEGER DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
DECLARE
  v_run ai_ops.planner_runs%ROWTYPE;
  v_settings ai_ops.settings%ROWTYPE;
  v_snapshot ai_ops.snapshots%ROWTYPE;
  v_credit_capture ai_ops.snapshot_domain_captures%ROWTYPE;
  v_sales_capture ai_ops.snapshot_domain_captures%ROWTYPE;
  v_customer_capture ai_ops.snapshot_domain_captures%ROWTYPE;
  v_limit INTEGER;
  v_credit_demand INTEGER := 0;
  v_sales_demand INTEGER := 0;
  v_customer_demand INTEGER := 0;
  v_allocations JSONB := '{}'::JSONB;
  v_credit_alloc INTEGER := 0;
  v_sales_alloc INTEGER := 0;
  v_customer_alloc INTEGER := 0;
  v_existing_domain_budget INTEGER := 0;
  v_existing_cases INTEGER := 0;
  v_remaining INTEGER := 0;
  v_context_bound BOOLEAN := false;
  v_customer_has_candidate BOOLEAN := false;
  v_two_domain_build JSONB;
  v_customer_build JSONB;
  v_total_cases INTEGER := 0;
  v_domain_captures JSONB := '[]'::JSONB;
BEGIN
  PERFORM pg_advisory_xact_lock(hashtextextended('ai_ops:operational_snapshot:'||p_run_id::TEXT,0));

  SELECT * INTO v_run
  FROM ai_ops.planner_runs
  WHERE id=p_run_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'ai_ops planner run not found: %',p_run_id;
  END IF;

  SELECT * INTO v_settings
  FROM ai_ops.settings
  WHERE singleton=true;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'AI Operations settings are not initialized';
  END IF;

  v_limit:=LEAST(
    GREATEST(COALESCE(p_case_limit,v_settings.max_cases_per_snapshot),1),
    v_settings.max_cases_per_snapshot
  );

  SELECT * INTO v_snapshot
  FROM ai_ops.snapshots
  WHERE run_id=p_run_id;

  IF FOUND THEN
    SELECT * INTO v_credit_capture
    FROM ai_ops.snapshot_domain_captures dc
    WHERE dc.snapshot_id=v_snapshot.id AND dc.domain='receivables';
    SELECT * INTO v_sales_capture
    FROM ai_ops.snapshot_domain_captures dc
    WHERE dc.snapshot_id=v_snapshot.id AND dc.domain='sales';
    SELECT * INTO v_customer_capture
    FROM ai_ops.snapshot_domain_captures dc
    WHERE dc.snapshot_id=v_snapshot.id AND dc.domain='customer_health';

    IF v_credit_capture.snapshot_id IS NULL OR v_sales_capture.snapshot_id IS NULL THEN
      RAISE EXCEPTION 'existing operational snapshot is missing Receivables/Sales capture markers';
    END IF;

    IF v_customer_capture.snapshot_id IS NOT NULL THEN
      v_total_cases:=v_credit_capture.case_count+v_sales_capture.case_count+v_customer_capture.case_count;
      IF v_total_cases>v_limit THEN
        RAISE EXCEPTION 'existing snapshot case count % exceeds global limit %',v_total_cases,v_limit;
      END IF;

      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'domain',dc.domain,
        'capture_status',dc.capture_status,
        'case_count',dc.case_count,
        'evidence_bytes',dc.evidence_bytes,
        'capture_version',dc.capture_version,
        'metadata',dc.metadata
      ) ORDER BY CASE dc.domain WHEN 'receivables' THEN 0 WHEN 'sales' THEN 1 WHEN 'customer_health' THEN 2 ELSE 3 END,dc.domain),'[]'::JSONB)
      INTO v_domain_captures
      FROM ai_ops.snapshot_domain_captures dc
      WHERE dc.snapshot_id=v_snapshot.id;

      RETURN jsonb_build_object(
        'snapshot_id',v_snapshot.id,
        'run_id',p_run_id,
        'business_date',v_run.business_date,
        'global_case_limit',v_limit,
        'captured_cases',v_total_cases,
        'idempotent_reuse',true,
        'domain_captures',v_domain_captures
      );
    END IF;

    v_context_bound:=NULLIF(v_run.result_summary->>'worker_context_hash','') IS NOT NULL;
    IF v_context_bound THEN
      RAISE EXCEPTION 'existing snapshot is already bound to a worker context and cannot add Customer Health';
    END IF;

    v_existing_cases:=v_credit_capture.case_count+v_sales_capture.case_count;
    IF v_existing_cases>v_limit THEN
      RAISE EXCEPTION 'existing snapshot case count % exceeds global limit %',v_existing_cases,v_limit;
    END IF;

    SELECT COUNT(*)::INTEGER INTO v_customer_demand
    FROM ai_ops.customer_health_candidates(v_run.business_date,2000);

    v_remaining:=GREATEST(v_limit-v_existing_cases,0);
    v_customer_alloc:=LEAST(v_customer_demand,v_remaining);
  ELSE
    IF v_run.status NOT IN ('pending','claimed','reasoning') THEN
      RAISE EXCEPTION 'planner run status does not allow snapshot build: %',v_run.status;
    END IF;

    SELECT LEAST(COUNT(*)::INTEGER,v_limit)
    INTO v_credit_demand
    FROM public.sales_orders so
    WHERE so.status IN (
      'delivered'::public.sales_order_status,
      'partially_delivered'::public.sales_order_status
    )
      AND so.payment_terms IN ('credit','mixed')
      AND so.delivered_at IS NOT NULL
      AND so.due_date IS NOT NULL
      AND so.due_date<v_run.business_date
      AND GREATEST(
        0::NUMERIC,
        COALESCE(so.total_amount,0)
          -COALESCE(so.paid_amount,0)
          -COALESCE(so.returned_amount,0)
      )>0;

    SELECT COUNT(*)::INTEGER INTO v_sales_demand
    FROM ai_ops.sales_target_candidates(v_run.business_date,v_limit);

    SELECT COUNT(*)::INTEGER INTO v_customer_demand
    FROM ai_ops.customer_health_candidates(v_run.business_date,2000);

    v_allocations:=ai_ops.allocate_domain_case_budget(
      v_limit,
      jsonb_build_object(
        'receivables',v_credit_demand,
        'sales',v_sales_demand,
        'customer_health',v_customer_demand
      ),
      ARRAY['receivables','sales','customer_health']::TEXT[]
    );

    v_credit_alloc:=COALESCE((v_allocations->>'receivables')::INTEGER,0);
    v_sales_alloc:=COALESCE((v_allocations->>'sales')::INTEGER,0);
    v_customer_alloc:=COALESCE((v_allocations->>'customer_health')::INTEGER,0);
    v_existing_domain_budget:=v_credit_alloc+v_sales_alloc;

    -- Reuse the reviewed two-domain builder as a bounded primitive. Its own
    -- fair spill policy is equivalent inside the reserved Receivables+Sales
    -- budget, while Customer Health capacity remains reserved globally.
    v_two_domain_build:=ai_ops.build_operational_snapshot_two_domain_v1(
      p_run_id,
      GREATEST(v_existing_domain_budget,1)
    );

    SELECT * INTO v_snapshot
    FROM ai_ops.snapshots
    WHERE run_id=p_run_id;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'snapshot missing after preserved two-domain base build';
    END IF;

    SELECT * INTO v_credit_capture
    FROM ai_ops.snapshot_domain_captures dc
    WHERE dc.snapshot_id=v_snapshot.id AND dc.domain='receivables';
    SELECT * INTO v_sales_capture
    FROM ai_ops.snapshot_domain_captures dc
    WHERE dc.snapshot_id=v_snapshot.id AND dc.domain='sales';

    IF v_credit_capture.snapshot_id IS NULL OR v_sales_capture.snapshot_id IS NULL THEN
      RAISE EXCEPTION 'preserved two-domain builder did not produce required capture markers';
    END IF;

    v_existing_cases:=v_credit_capture.case_count+v_sales_capture.case_count;
    v_remaining:=GREATEST(v_limit-v_existing_cases,0);

    -- Spill any unused reserved capacity to Customer Health, never by comparing
    -- severity across domains.
    v_customer_alloc:=LEAST(v_customer_demand,v_remaining);
  END IF;

  IF v_customer_alloc>0 THEN
    v_customer_build:=ai_ops.refresh_customer_health_cases(
      v_snapshot.id,
      v_run.business_date,
      v_customer_alloc
    );
  ELSE
    v_customer_has_candidate:=v_customer_demand>0;
    INSERT INTO ai_ops.snapshot_domain_captures(
      snapshot_id,domain,capture_version,source_as_of,business_date,
      case_count,evidence_bytes,capture_status,metadata
    ) VALUES (
      v_snapshot.id,
      'customer_health',
      'customer-health-v1',
      v_snapshot.data_as_of,
      v_run.business_date,
      0,
      0,
      CASE WHEN v_customer_has_candidate THEN 'partial' ELSE 'completed' END,
      jsonb_build_object(
        'capture_marker_written',true,
        'case_types',jsonb_build_array('reactivation_gap','high_value_reactivation_opportunity'),
        'global_budget_exhausted',v_customer_has_candidate,
        'global_case_limit',v_limit,
        'allocated_case_limit',0,
        'has_more',v_customer_has_candidate,
        'target_recalculation_performed',false
      )
    );
  END IF;

  SELECT * INTO v_customer_capture
  FROM ai_ops.snapshot_domain_captures dc
  WHERE dc.snapshot_id=v_snapshot.id AND dc.domain='customer_health';
  SELECT * INTO v_credit_capture
  FROM ai_ops.snapshot_domain_captures dc
  WHERE dc.snapshot_id=v_snapshot.id AND dc.domain='receivables';
  SELECT * INTO v_sales_capture
  FROM ai_ops.snapshot_domain_captures dc
  WHERE dc.snapshot_id=v_snapshot.id AND dc.domain='sales';

  IF v_credit_capture.snapshot_id IS NULL
     OR v_sales_capture.snapshot_id IS NULL
     OR v_customer_capture.snapshot_id IS NULL THEN
    RAISE EXCEPTION 'operational snapshot did not produce all three immutable domain capture markers';
  END IF;

  v_total_cases:=v_credit_capture.case_count+v_sales_capture.case_count+v_customer_capture.case_count;
  IF v_total_cases>v_limit THEN
    RAISE EXCEPTION 'multi-domain snapshot case count % exceeds global limit %',v_total_cases,v_limit;
  END IF;

  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'domain',dc.domain,
    'capture_status',dc.capture_status,
    'case_count',dc.case_count,
    'evidence_bytes',dc.evidence_bytes,
    'capture_version',dc.capture_version,
    'metadata',dc.metadata
  ) ORDER BY CASE dc.domain WHEN 'receivables' THEN 0 WHEN 'sales' THEN 1 WHEN 'customer_health' THEN 2 ELSE 3 END,dc.domain),'[]'::JSONB)
  INTO v_domain_captures
  FROM ai_ops.snapshot_domain_captures dc
  WHERE dc.snapshot_id=v_snapshot.id;

  RETURN jsonb_build_object(
    'snapshot_id',v_snapshot.id,
    'run_id',p_run_id,
    'business_date',v_run.business_date,
    'global_case_limit',v_limit,
    'credit_demand',v_credit_demand,
    'sales_demand',v_sales_demand,
    'customer_health_demand',v_customer_demand,
    'credit_allocated_limit',v_credit_alloc,
    'sales_allocated_limit',v_sales_alloc,
    'customer_health_allocated_limit',v_customer_alloc,
    'captured_cases',v_total_cases,
    'idempotent_reuse',false,
    'domain_captures',v_domain_captures,
    'target_recalculation_performed',false
  );
END;
$$;

REVOKE ALL ON FUNCTION ai_ops.build_operational_snapshot(UUID,INTEGER)
  FROM PUBLIC,anon,authenticated,service_role;

COMMENT ON FUNCTION ai_ops.allocate_domain_case_budget(INTEGER,JSONB,TEXT[]) IS
  'Deterministic round-robin water-fill allocator for one global multi-domain case budget; does not compare cross-domain severity.';
COMMENT ON FUNCTION ai_ops.build_operational_snapshot(UUID,INTEGER) IS
  'Canonical operational snapshot builder for Receivables, Sales and Customer Health using one immutable snapshot and one global case budget.';

RESET lock_timeout;
RESET statement_timeout;