-- ============================================================================
-- AI Operations Planner — Customer Health Immutable Snapshot Capture
--
-- DESIGN-TIME MIGRATION ONLY. No operational business table is mutated.
-- Freezes Customer Health cases, responsibility evidence, bounded governed
-- context and trust metadata into the shared immutable planner snapshot.
-- ============================================================================

SET lock_timeout = '5s';
SET statement_timeout = '60s';

CREATE OR REPLACE FUNCTION ai_ops.refresh_customer_health_cases(
  p_snapshot_id UUID,
...
DECLARE
  ...
  v_context JSONB;
  v_count INTEGER := 0;
  v_reactivation_count INTEGER := 0;
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE dc.snapshot_id=p_snapshot_id AND dc.domain='customer_health'
  ) THEN
    RAISE EXCEPTION 'ai_ops customer_health domain already captured: %',p_snapshot_id;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'ai_ops customer_health evidence exists without capture marker: %',p_snapshot_id;
  END IF;
  ...
  INTO v_has_more
  FROM ai_ops.customer_health_candidates(
    p_business_date,
    LEAST(v_effective_limit+1,2000)
  );
  ...
  FOR v_candidate IN
    SELECT *
    FROM ai_ops.customer_health_candidates(p_business_date,v_effective_limit)
  LOOP
    v_count:=v_count+1;
    IF v_candidate.case_type='reactivation_gap' THEN
      v_reactivation_count:=v_reactivation_count+1;
    ELSE
    ...
    INSERT INTO ai_ops.cases(
      ...
      v_candidate.case_key,p_snapshot_id,p_snapshot_id,'customer_health',
      v_candidate.case_type,'customer',v_candidate.customer_id,
    ...
    WHERE oc.status='active'
      ...
      AND (
        (oc.subject_type='customer' AND oc.subject_id=v_candidate.customer_id)
        OR (
          v_candidate.target_id IS NOT NULL
          AND oc.subject_type='target'
          AND oc.subject_id=v_candidate.target_id
        )
        OR (
          v_candidate.assigned_rep_user_id IS NOT NULL
          AND oc.subject_type='employee'
          AND oc.subject_id=(
            SELECT he.id
            FROM public.hr_employees he
            WHERE he.user_id=v_candidate.assigned_rep_user_id
            ORDER BY he.updated_at DESC NULLS LAST,he.id
            LIMIT 1
          )
        )
        OR (
          v_candidate.target_scope='department'
          AND oc.subject_type='department'
          AND oc.subject_id=v_candidate.target_scope_id
        )
        OR (
          v_candidate.target_scope='branch'
    ...
    v_case_payload_bytes:=octet_length(convert_to(
      jsonb_build_object(
        ...
        'case_type',v_candidate.case_type,
        'attention_class',v_candidate.attention_class,
    ...
    INSERT INTO ai_ops.snapshot_cases(
      ...
      'customer_health',v_candidate.case_type,'customer',v_candidate.customer_id,
  ...
  INSERT INTO ai_ops.snapshot_domain_captures(
    ...
    p_snapshot_id,'customer_health','customer-health-v1',
    v_snapshot.data_as_of,p_business_date,v_count,v_evidence_bytes,
    v_capture_status,
    jsonb_build_object(
      'case_types',jsonb_build_array('reactivation_gap','high_value_reactivation_opportunity'),
      'reactivation_gap_count',v_reactivation_count,
      'high_value_opportunity_count',v_opportunity_count,
...
REVOKE ALL ON FUNCTION ai_ops.refresh_customer_health_cases(UUID,DATE,INTEGER)
  FROM PUBLIC,anon,authenticated,service_role;

COMMENT ON FUNCTION ai_ops.refresh_customer_health_cases(UUID,DATE,INTEGER) IS
  'Single-shot Customer Health capture freezing reactivation gaps and high-value dormant opportunities into the shared immutable snapshot.';
