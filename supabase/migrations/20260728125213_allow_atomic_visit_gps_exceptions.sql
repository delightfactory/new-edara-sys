...
BEGIN
  SELECT * INTO v_type FROM public.activity_types
  WHERE id = NEW.type_id AND is_active = true;
  IF NOT FOUND THEN RAISE EXCEPTION 'نوع النشاط غير موجود أو غير نشط'; END IF;

  IF NEW.visit_plan_item_id IS NOT NULL THEN
    SELECT * INTO v_visit_item FROM public.visit_plan_items
    ...
    v_is_atomic_visit_gps_exception :=
      ...
      AND v_visit_item.gps_validation_status IN ('no_coordinates','failed_accuracy','failed_distance');
  END IF;
  ...
  IF NEW.visit_plan_item_id IS NOT NULL THEN
    ...
    END IF;
    IF v_visit_item.id IS NULL THEN RAISE EXCEPTION 'بند الزيارة المحدد غير موجود'; END IF;
