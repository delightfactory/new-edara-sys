-- ============================================================================
-- AI Operations Planner — Field Execution Human-Approved Work Bridge
--
-- DESIGN-TIME MIGRATION ONLY.
-- Reviewed Field Execution decisions may create Work only. Visit plans/items,
-- activities, customer records and field evidence are never mutated here.
-- ============================================================================
SET lock_timeout = '5s';
SET statement_timeout = '60s';

-- Make the exact visit-day entity first-class in Work links.
...
CREATE OR REPLACE FUNCTION private.work_create_ai_reviewed_field_execution_task(
  p_decision_id UUID,
...
BEGIN
  ...
  -- Same-transaction revalidation immediately before any Work mutation.
  ...
  IF v_sc.domain<>'field_execution' OR v_sc.case_type<>'overdue_visit_day' THEN
    RAISE EXCEPTION 'Field Execution Work bridge received unsupported frozen case';
  END IF;
  ...
  IF v_visit_plan_id IS NULL OR NOT private.work_link_entity_exists('visit_plan',v_visit_plan_id) THEN
    RAISE EXCEPTION 'visit plan no longer exists or is unsupported by Work link contract';
  END IF;
  IF v_rep_employee_id IS NULL OR NOT private.work_link_entity_exists('employee',v_rep_employee_id) THEN
    RAISE EXCEPTION 'plan employee no longer exists or is unsupported by Work link contract';
  END IF;
  ...
  v_title:=left(
    'استكمال وإغلاق يومية زيارات متأخرة — '||
    COALESCE(NULLIF(v_sc.responsibility_evidence->>'rep_name',''),'مندوب')||' — '||
    COALESCE(NULLIF(v_sc.facts->>'plan_date',''),'يومية'),240
  );
  v_description:=left(concat_ws(E'\n',
    'إجراء تشغيلي ناتج عن مراجعة معتمدة ليومية زيارات سابقة ما زالت تحتوي بنودًا معلقة.',
    'راجع البنود المعلقة وحدد لكل بند الإجراء الصحيح من داخل مسار الزيارات المعتمد: تنفيذ ما يزال صالحًا، إعادة الجدولة، أو الإغلاق الإداري وفق الصلاحيات والحالة الفعلية.',
    'سجّل نتيجة المتابعة والخطوة التالية داخل Work. هذا التكليف لا يغيّر حالة أي زيارة أو نشاط تلقائيًا ولا يفترض سبب عدم التنفيذ.'
  ),2000);
  ...
  INSERT INTO public.work_items(
    ...
    v_now,1,jsonb_build_object(
      'origin','ai_ops','ai_domain','field_execution','ai_case_type',v_sc.case_type,
      ...
      'management_rationale_exposed',false,'visit_mutation_performed',false
    )
  ...
  PERFORM private.ai_ops_append_work_system_event(
    ...
    jsonb_build_object(
      'source_kind','system','source_key',v_source_key,'origin','ai_ops','domain','field_execution',
  ...
  INSERT INTO public.work_links(
    ...
    v_work.id,'visit_plan',v_visit_plan_id,'primary',
    left('يومية زيارات '||COALESCE(v_sc.facts->>'plan_date',''),240),NULL
  )
  ...
  INSERT INTO public.work_links(
    ...
    v_work.id,'employee',v_rep_employee_id,'assigned_field_rep',
    left(COALESCE(v_sc.responsibility_evidence->>'rep_name','مندوب'),240),NULL
  )
  ON CONFLICT(work_item_id,entity_type,entity_id,relation_type) DO NOTHING;
  ...
  PERFORM private.ai_ops_append_work_system_event(
    ...
    jsonb_build_object(
      'source','ai_ops_human_approved','domain','field_execution','case_type',v_sc.case_type,
  ...
  SET committed_work_item_id=v_work.id,committed_at=v_now,commit_status='committed',
      validation_detail=validation_detail||jsonb_build_object(
        ...
        'human_review_id',v_review.id,'approved_execution_by',p_approved_execution_by,
        'domain','field_execution','stage_only',false,'visit_mutation_performed',false
      ),updated_at=v_now
  WHERE id=p_decision_id;
  ...
  SET checkpoint='reviewed_work_committed',work_created_count=work_created_count+1,
      result_summary=result_summary||jsonb_build_object(
        ...
        'last_commit_at',v_now,'last_commit_domain','field_execution'
      ),updated_at=v_now
  ...
  RETURN jsonb_build_object(
    ...
    'domain','field_execution','case_type',v_sc.case_type,
    'operational_mutation','work_create_only','visit_mutation_performed',false
  );
END;
$$;

REVOKE ALL ON FUNCTION private.work_create_ai_reviewed_field_execution_task(UUID,UUID)
  FROM PUBLIC,anon,authenticated,service_role;

ALTER FUNCTION private.work_create_ai_reviewed_task(UUID,UUID)
  RENAME TO work_create_ai_reviewed_task_four_domain_v1;
REVOKE ALL ON FUNCTION private.work_create_ai_reviewed_task_four_domain_v1(UUID,UUID)
  FROM PUBLIC,anon,authenticated,service_role;
...
BEGIN
  ...
  IF v_domain='field_execution' THEN
    RETURN private.work_create_ai_reviewed_field_execution_task(
      p_decision_id,p_approved_execution_by
    );
  END IF;
  RETURN private.work_create_ai_reviewed_task_four_domain_v1(
    p_decision_id,p_approved_execution_by
  );
END;
$$;
...
COMMENT ON FUNCTION private.work_create_ai_reviewed_field_execution_task(UUID,UUID) IS
  'Human-approved Field Execution CREATE_WORK bridge with kill switches, immutable review fingerprint, same-transaction revalidation and exact visit-plan/employee links; field records remain read-only.';
COMMENT ON FUNCTION private.work_create_ai_reviewed_task(UUID,UUID) IS
  'Canonical reviewed Work bridge dispatcher for Receivables, Sales, Customer Health, Inventory and Field Execution.';
