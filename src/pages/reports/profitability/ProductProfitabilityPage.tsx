...
import { useGrossProfitByProduct } from '@/hooks/useProfitability'
...
export default function ProductProfitabilityPage() {
  ...
  React.useEffect(() => setTitle('ربحية المنتجات (هامش الربح الإجمالي)'), [setTitle])

  const [dateRange, setDateRange] = useState<DateRange>(getInitialDateRange)

  const { data, isLoading } = useGrossProfitByProduct({
  ...
  // Using entity_id_display which maps from product_id
  const columns: ColumnDef<any>[] = [
    { key: 'entity_id_display', header: 'المنتج', align: 'right' },
    { key: 'net_quantity', header: 'الكمية' },
  ...
  const exportCSV = () => {
    if (!data) return
    downloadAsCSV(data, `Products_Profitability_${dateRange.from}_${dateRange.to}`, [
      { key: 'entity_id_display', label: 'المنتج' },
      { key: 'net_quantity', label: 'الكمية' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 'var(--space-4)' }}>
        <div>
          <h2 style={{ fontSize: 'var(--text-xl)', fontWeight: 700, margin: 0 }}>تحليل المنتجات</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: 'var(--text-sm)', marginTop: '4px' }}>
            يتجاهل هذا التقرير التكاليف غير المباشرة (Operating & Payroll). الربح المعروض هنا هو هامش الربح المباشر فقط.
          </p>
        </div>
      ...
      <FinancialBreakdownTable
        data={data || []}
        columns={columns}
        rowKey={row => row.product_id ?? 'unknown'}
        isLoading={isLoading}
      />
    </div>
  )
