 ...
 *
 * النموذج التشغيلي الصحيح:
 * - يختار HR فترة رواتب
 * - يضغط "حساب وتثبيت" → يشغّل prepareTargetRewardPayouts(periodId)
 * - يرى السجلات الناتجة (committed) كتاريخ قابل للتصفية
 * - لا يوجد pending review flow — العملية تثبت مباشرةً
 */
...
import {
  useTargetPayouts, usePrepareTargetPayouts, useHRPayrollPeriods,
} from '@/hooks/useQueryHooks'
import { toast } from 'sonner'
import { Play, ChevronRight, Gift, AlertCircle, Filter, TrendingUp, CheckCircle } from 'lucide-react'
...
export default function TargetPayoutsPage() {
  ...
  }), [selectedPeriodId, filterStatus])

  const { data: payouts = [], isLoading: loadingPayouts } = useTargetPayouts(payoutFilters)
  const preparePayouts = usePrepareTargetPayouts()

  // ── Stats
  const totalPayout = payouts.reduce((sum, p) => sum + (p.payout_amount ?? 0), 0)
  ...
  const handleRun = async () => {
    if (!selectedPeriodId) {
      toast.error('اختر فترة الرواتب أولاً')
      return
    }
    setRunning(true)
  ...
  return (
    <div className="page-container animate-enter">
      ...
      <div className="edara-card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        {loadingPayouts ? (
          <div style={{ padding: '24px' }}>
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="skeleton" style={{ height: 50, marginBottom: '8px', borderRadius: '6px' }} />
            ))}
          </div>
        ) : payouts.length === 0 ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid var(--border-primary)', background: 'var(--bg-surface-2)' }}>
                  <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 700, color: 'var(--text-muted)', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>الهدف</th>
                  <th style={{ padding: '12px', textAlign: 'right', fontWeight: 700, color: 'var(--text-muted)', fontSize: '11px' }}>الفترة / التاريخ</th>
                  <th style={{ padding: '12px', textAlign: 'center', fontWeight: 700, color: 'var(--text-muted)', fontSize: '11px' }}>الإنجاز</th>
                  <th style={{ padding: '12px', textAlign: 'center', fontWeight: 700, color: 'var(--text-muted)', fontSize: '11px' }}>الشريحة</th>
                  <th style={{ padding: '12px', textAlign: 'center', fontWeight: 700, color: 'var(--text-muted)', fontSize: '11px' }}>المبلغ</th>
                  <th style={{ padding: '12px', textAlign: 'center', fontWeight: 700, color: 'var(--text-muted)', fontSize: '11px' }}>حالة الصرف</th>
                </tr>
              </thead>
              <tbody>
                {(payouts as any[]).map((p: any) => (
                  <tr key={p.id} style={{ borderBottom: '1px solid var(--border-secondary)', transition: 'background 0.15s' }}
                    className="tp-table-row">
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ fontWeight: 600, color: 'var(--text-primary)', marginBottom: '2px' }}>
                        {p.target?.name ?? '—'}
                      </div>
                      <div style={{ fontSize: '11px', color: 'var(--color-primary)', cursor: 'pointer' }}
      ...
      <style>{`
        ...
        .tp-table-header { display: flex; align-items: center; justify-content: space-between; padding: var(--space-4) var(--space-5); border-bottom: 1px solid var(--border-primary); }
