...
export default function CallPlansPage() {
  ...
  const [cancelTarget, setCancelTarget] = useState<CallPlan | null>(null)
  const [cancelReason, setCancelReason] = useState('')
  const [confirmTarget, setConfirmTarget] = useState<CallPlan | null>(null)
  ...
  const { data: result, isLoading: loading } = useCallPlans(queryParams)
  const plans      = result?.data     ?? []
  const totalPages = result?.totalPages ?? 1
  const totalCount = result?.count    ?? 0

  const canCreate  = can(PERMISSIONS.CALL_PLANS_CREATE)
  ...
  const handleConfirm = () => {
    ...
    confirmPlan.mutate(confirmTarget.id, {
      onSuccess: () => { toast.success('تم تأكيد خطة المكالمات'); setConfirmTarget(null) },
  ...
  return (
    <div className="page-container animate-enter">
      ...
      <div className="act-table-view edara-card" style={{ overflow: 'auto' }}>
        <DataTable<CallPlan>
          columns={[
            ...
            { key: 'employee', label: 'المندوب', hideOnMobile: true, render: p => p.employee?.full_name || '—' },
            { key: 'status', label: 'الحالة', render: p => <ActivityStatusBadge planStatus={p.status} size="sm" /> },
            {
              ...
              render: p => (
                <div>
                  ...
                  <div style={{ height: 4, background: 'var(--bg-surface-2)', borderRadius: 99, marginTop: 4, overflow: 'hidden', width: 80 }}>
                    <div style={{ height: '100%', width: `${Math.min(p.completion_pct, 100)}%`, background: 'var(--color-success)', borderRadius: 99 }} />
                  </div>
                </div>
              ),
            },
            {
              ...
              render: p => (
                <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                  <Button variant="ghost" size="sm" onClick={() => navigate(`/activities/call-plans/${p.id}`)}>
                    <Eye size={14} />
                  </Button>
      ...
      <ResponsiveModal open={!!cancelTarget} onClose={() => { setCancelTarget(null); setCancelReason('') }}
        title="إلغاء خطة المكالمات" disableOverlayClose={processing}
        footer={<>
          <Button variant="secondary" onClick={() => { setCancelTarget(null); setCancelReason('') }} disabled={processing}>تراجع</Button>
