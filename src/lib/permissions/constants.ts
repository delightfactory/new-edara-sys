...
export const PERMISSIONS = {
  ...
  AUTH_USER_PERMISSIONS_MANAGE: 'auth.user_permissions.manage',
  ...
  SALES_RETURNS_CANCEL:     'sales.returns.cancel',
  ...
  // Procurement invoices & returns
  PROCUREMENT_INVOICES_READ:    'procurement.invoices.read',
  PROCUREMENT_INVOICES_CREATE:  'procurement.invoices.create',
  PROCUREMENT_INVOICES_RECEIVE: 'procurement.invoices.receive',
  PROCUREMENT_INVOICES_BILL:    'procurement.invoices.bill',
  PROCUREMENT_INVOICES_PAY:     'procurement.invoices.pay',
  PROCUREMENT_INVOICES_CANCEL:  'procurement.invoices.cancel',
  PROCUREMENT_RETURNS_READ:     'procurement.returns.read',
  PROCUREMENT_RETURNS_CREATE:   'procurement.returns.create',
  PROCUREMENT_RETURNS_CONFIRM:  'procurement.returns.confirm',

  // Finance
  FINANCE_VIEW_COSTS:         'finance.view_costs',
  FINANCE_READ:               'finance.read',
  FINANCE_CREDIT_MANAGE:      'finance.credit.manage',
  ...
  HR_ATTENDANCE_CHECKIN:'hr.attendance.checkin',
  HR_LEAVES_READ:       'hr.leaves.read',
  HR_LEAVES_CREATE:     'hr.leaves.create',
  HR_LEAVES_REQUEST:    'hr.leaves.request',
  HR_LEAVES_APPROVE:    'hr.leaves.approve',
  HR_ADVANCES_READ:     'hr.advances.read',
  HR_ADVANCES_CREATE:   'hr.advances.create',
  HR_ADVANCES_APPROVE:  'hr.advances.approve',
  HR_ADJUSTMENTS_READ:  'hr.adjustments.read',
  HR_ADJUSTMENTS_CREATE:'hr.adjustments.create',
  HR_ADJUSTMENTS_APPROVE:'hr.adjustments.approve',
  HR_PERMISSIONS_APPROVE:'hr.permissions.approve',
  HR_COMMISSIONS_CREATE:'hr.commissions.create',
  HR_SETTINGS_UPDATE:   'hr.settings.update',
  ...
  VISIT_PLANS_CLOSE_DAY:      'visit_plans.close_day',
  ...
  TARGETS_READ_LEGACY:        'targets.read',        // مفتاح قديم موجود في بعض بيانات الأدوار
  TARGETS_VIEW:               'targets.view',
  TARGETS_REWARDS_VIEW:       'targets.rewards.view',
  TARGETS_REWARDS_CONFIGURE:  'targets.rewards.configure',
...
export const PERMISSION_GROUPS = [
  {
    ...
    permissions: [
      ...
      { key: PERMISSIONS.AUTH_USER_PERMISSIONS_MANAGE, label: 'إدارة صلاحيات المستخدم الفردية' },
  ...
  },
  {
    ...
    label: 'المبيعات',
    permissions: [
      { key: PERMISSIONS.SALES_READ,               label: 'دخول وحدة المبيعات' },
      ...
      { key: PERMISSIONS.SALES_ORDERS_DELETE,      label: 'حذف طلب' },
      ...
      { key: PERMISSIONS.SALES_RETURNS_CANCEL,     label: 'إلغاء مرتجع' },
  ...
  },
  {
    ...
    permissions: [
      ...
      { key: PERMISSIONS.PROCUREMENT_INVOICES_READ,    label: 'عرض فواتير المشتريات' },
      { key: PERMISSIONS.PROCUREMENT_INVOICES_CREATE,  label: 'إنشاء فاتورة مشتريات' },
      { key: PERMISSIONS.PROCUREMENT_INVOICES_RECEIVE, label: 'استلام فاتورة مشتريات' },
      { key: PERMISSIONS.PROCUREMENT_INVOICES_BILL,    label: 'الاعتماد المالي لفاتورة' },
      { key: PERMISSIONS.PROCUREMENT_INVOICES_PAY,     label: 'سداد فاتورة مورد' },
      { key: PERMISSIONS.PROCUREMENT_INVOICES_CANCEL,  label: 'إلغاء فاتورة مشتريات' },
      { key: PERMISSIONS.PROCUREMENT_RETURNS_READ,     label: 'عرض مرتجعات المشتريات' },
      { key: PERMISSIONS.PROCUREMENT_RETURNS_CREATE,   label: 'إنشاء مرتجع مشتريات' },
      { key: PERMISSIONS.PROCUREMENT_RETURNS_CONFIRM,  label: 'اعتماد مرتجع مشتريات' },
    ]
  },
  {
    id: 'finance',
    label: 'المالية',
    permissions: [
      { key: PERMISSIONS.FINANCE_READ,               label: 'دخول الوحدة المالية' },
      { key: PERMISSIONS.FINANCE_VIEW_COSTS,         label: 'عرض التكاليف وهوامش الربح' },
      { key: PERMISSIONS.FINANCE_CREDIT_MANAGE,      label: 'إدارة الائتمان' },
      ...
      { key: PERMISSIONS.FINANCE_JOURNALS_CREATE,    label: 'إنشاء قيد محاسبي (الإصدار الجديد)' },
  ...
  },
  {
    ...
    permissions: [
      ...
      { key: PERMISSIONS.HR_EMPLOYEES_EDIT,    label: 'تعديل موظف' },
      { key: PERMISSIONS.HR_EMPLOYEES_UPDATE,  label: 'تحديث بيانات الموظفين' },
      { key: PERMISSIONS.HR_EMPLOYEES_DELETE,  label: 'حذف موظف' },
      { key: PERMISSIONS.HR_ATTENDANCE_READ,   label: 'عرض الحضور' },
      { key: PERMISSIONS.HR_ATTENDANCE_CREATE, label: 'إنشاء سجل حضور' },
      { key: PERMISSIONS.HR_ATTENDANCE_EDIT,   label: 'تعديل سجل حضور' },
      { key: PERMISSIONS.HR_ATTENDANCE_UPDATE, label: 'تحديث سجلات الحضور' },
      { key: PERMISSIONS.HR_ATTENDANCE_APPROVE,label: 'اعتماد الحضور' },
      { key: PERMISSIONS.HR_ATTENDANCE_CHECKIN,label: 'تسجيل الحضور والانصراف' },
      { key: PERMISSIONS.HR_LEAVES_READ,       label: 'عرض الإجازات' },
      { key: PERMISSIONS.HR_LEAVES_CREATE,     label: 'إنشاء إجازة' },
      { key: PERMISSIONS.HR_LEAVES_REQUEST,    label: 'طلب إجازة' },
      { key: PERMISSIONS.HR_LEAVES_APPROVE,    label: 'اعتماد الإجازات' },
      { key: PERMISSIONS.HR_ADVANCES_READ,     label: 'عرض السلف' },
      { key: PERMISSIONS.HR_ADVANCES_CREATE,   label: 'طلب أو إنشاء سلفة' },
      { key: PERMISSIONS.HR_ADVANCES_APPROVE,  label: 'اعتماد السلف' },
      { key: PERMISSIONS.HR_ADJUSTMENTS_READ,  label: 'عرض المكافآت والخصومات' },
      { key: PERMISSIONS.HR_ADJUSTMENTS_CREATE,label: 'إنشاء مكافأة أو خصم' },
      { key: PERMISSIONS.HR_ADJUSTMENTS_APPROVE,label: 'اعتماد المكافآت والخصومات' },
      { key: PERMISSIONS.HR_PERMISSIONS_APPROVE,label: 'اعتماد أذونات الموظفين' },
      { key: PERMISSIONS.HR_COMMISSIONS_CREATE,label: 'إنشاء العمولات' },
      { key: PERMISSIONS.HR_SETTINGS_UPDATE,   label: 'تعديل إعدادات الموارد البشرية' },
      { key: PERMISSIONS.HR_REPORTS_VIEW,      label: 'عرض تقارير الموارد البشرية' },
      ...
      { key: PERMISSIONS.HR_PAYROLL_DISBURSE,  label: 'صرف الرواتب' },
    ]
  },
  {
    id: 'reports',
  ...
  },
  {
    ...
    permissions: [
      { key: PERMISSIONS.ACTIVITIES_CREATE,        label: 'إنشاء نشاط' },
      { key: PERMISSIONS.ACTIVITIES_READ_OWN,      label: 'عرض أنشطتي' },
      ...
      { key: PERMISSIONS.VISIT_PLANS_CLOSE_DAY,      label: 'إغلاق يوم الزيارات' },
      ...
      { key: PERMISSIONS.TARGETS_READ_LEGACY,      label: 'عرض الأهداف (توافق)' },
      { key: PERMISSIONS.TARGETS_VIEW,             label: 'عرض وحدة الأهداف' },
      { key: PERMISSIONS.TARGETS_REWARDS_VIEW,     label: 'عرض مكافآت الأهداف' },
      { key: PERMISSIONS.TARGETS_REWARDS_CONFIGURE,label: 'إعداد مكافآت الأهداف' },
