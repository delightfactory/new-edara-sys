...
export function setQueueMember(input: {
  queueId: string
  userId: string
  memberRole: 'member' | 'triager' | 'manager'
  canTriage: boolean
  canAssign: boolean
  isActive: boolean
}) {
  return executeManagementAtomic<{ membership_id?: string; active: boolean }>('work_set_queue_member', {
    ...
    p_can_triage: input.canTriage,
    p_can_assign: input.canAssign,
    p_is_active: input.isActive,
  })
}
...
export function createRequestType(input: CreateRequestTypeInput) {
  return executeManagementAtomic<{ request_type_id: string; state_version: number }>('work_create_request_type', {
    ...
    p_name: input.name,
    p_description: input.description ?? null,
    p_target_queue_id: input.targetQueueId,
    p_expected_outcome_template: input.expectedOutcomeTemplate,
    p_intake_schema: input.intakeSchema,
    p_target_department_id: input.targetDepartmentId ?? null,
    p_default_priority: input.defaultPriority ?? null,
    p_default_visibility: input.defaultVisibility ?? 'standard',
    p_triage_sla_minutes: input.triageSlaMinutes ?? null,
    p_default_resolution_sla_minutes: input.defaultResolutionSlaMinutes ?? null,
    p_allow_requester_cancel: input.allowRequesterCancel ?? true,
  })
}

export function createApprovalTemplate(input: {
  code: string
  name: string
  description?: string | null
  definition: WorkApprovalDefinitionDraft
  publish: boolean
}) {
...
export function createWorkflowTemplate(input: {
  ...
  name: string
  description?: string | null
  definition: WorkWorkflowDefinitionDraft
  publish: boolean
}) {
...
export function createRecurrence(input: CreateRecurrenceInput) {
  return executeManagementAtomic<{ recurrence_definition_id: string; state_version: number }>('work_create_recurrence', {
    ...
    p_monthly_policy: input.monthlyPolicy ?? null,
    p_overlap_policy: input.overlapPolicy ?? 'strict',
    p_task_config: input.taskConfig ?? {},
    p_workflow_template_id: input.workflowTemplateId ?? null,
    p_workflow_version_policy: input.workflowVersionPolicy ?? null,
    p_workflow_input: input.workflowInput ?? {},
...
export function updateOperationalSettings(input: {
  ...
  dueAlertCooldownHours?: number
  followUpAlertCooldownHours?: number
  staleAlertCooldownHours?: number
  blockedAlertCooldownHours?: number
  ...
  return executeManagementAtomic<{ state_version: number; manager_escalation_enabled: boolean }>('work_update_operational_settings', {
    p_operation_id: operationId(),
    p_expected_version: input.expectedVersion,
    p_due_soon_hours: input.dueSoonHours ?? null,
    p_stale_after_hours: input.staleAfterHours ?? null,
