...
describe('AI Operations release hardening contract', () => {
  ...
  it('derives a conservative worker case selection from the hard byte budget without dropping selected cases', () => {
    expect(contextMigration).toContain('CREATE OR REPLACE FUNCTION ai_ops.compact_worker_json')
    expect(contextMigration).not.toContain('worker_json_has_meaningful_value')
    expect(contextMigration).toContain('v_context_case_limit')
    expect(contextMigration).toContain('v_context_envelope_reserve_bytes CONSTANT INTEGER := 20480')
    expect(contextMigration).toContain('v_compacted_case_budget_bytes CONSTANT INTEGER := 6144')
    expect(contextMigration).toContain('v_settings.max_worker_context_bytes - v_context_envelope_reserve_bytes')
    expect(contextMigration).toContain('v_required_domain_count')
    ...
    expect(contextMigration).toContain("cases_seen = v_total_cases")
    expect(contextMigration).toContain("v_hash := md5(v_body::TEXT)")
    expect(contextMigration).not.toContain('max_worker_context_bytes - 12288')
    expect(contextMigration).not.toContain('/ 1400')
  ...
  it('provides a guarded sequential migration and realistic clone context rehearsal', () => {
    ...
    expect(cloneContextGate).toContain('v_case_domain_count <> v_expected_case_domain_count')
    expect(cloneContextGate).toContain("COALESCE((dc->>'case_count')::INTEGER, 0) > 0")
    expect(cloneContextGate).toContain("COALESCE((dc->>'has_more')::BOOLEAN, false)")
    expect(cloneContextGate).toContain("COALESCE((dc->>'global_budget_exhausted')::BOOLEAN, false)")
    expect(cloneContextGate).toContain('do not exactly match % demand-bearing domain captures')
    expect(cloneContextGate).toContain('CREATE FUNCTION pg_temp.worker_json_has_meaningful_value')
    expect(cloneContextGate).toContain("p_value = 'null'::JSONB")
    expect(cloneContextGate).toContain("e.key NOT IN ('_truncated', '_truncated_depth', '_total_items')")
    expect(cloneContextGate).toContain("btrim(p_value #>> '{}') <> ''")
    expect(cloneContextGate).toContain("RETURN v_type IN ('number', 'boolean')")
    expect(cloneContextGate).toContain("c ? 'responsibility_evidence'")
    expect(cloneContextGate).toContain("c ? 'operational_context'")
    expect(cloneContextGate).toContain("pg_temp.worker_json_has_meaningful_value(c->'facts')")
    expect(cloneContextGate).toContain("pg_temp.worker_json_has_meaningful_value(c->'responsibility_evidence')")
    expect(cloneContextGate).toContain("pg_temp.worker_json_has_meaningful_value(c->'operational_context')")
